...
app = Flask(__name__)
try:
    with sqlite3.connect("dashcam.db") as conn:
        cursor = conn.cursor()
...
@app.route("/dashcam/new", methods=["GET", "POST"])
def pageNewDashcam():
...
@app.route("/dashcam/update/<ip>", methods=["GET", "POST"])
